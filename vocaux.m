...
for k_block2 = 0:length(rg_full_block)-1
    ...
    [cr_block2,     p_block2]     = oli_corr(nm_block2,...
                                             nm_instru_block);
    [cr_max_block2,  n_max_block2]  = max(cr_block2); %plus grand du bloc2.
    [cr2_max_block2, n2_max_block2] = max(cr_max_block2); %plus grand de la stéréo.
    p_max_block2 = p_block2(n_max_block2(n2_max_block2));

    rg_instru2 =...
        max(1,...
            1       +rg_full_block(1+k_block2)-p_max_block2-n_tail):...
        min(n_instru,...
            n_block2+rg_full_block(1+k_block2)-p_max_block2+n_tail);
    ...
    for k_block = (0:c_block2-1)
	    ...
	    % k = p_corr + N_y - 1
            n_block_start = ...
                1 + rg_full_block(1+k_block2+c_block2*k_block2);
	    n_block_end = ...
                min(n_full, n_block_start + n_block - 1);
	    rg_block = n_block_start:n_block_end;
	    au_block = au_full(rg_block,:);
	    [cv_block_instru2, p_block_instru2] = oli_cov(au_block,...
						          au_instru2);
	    ...
	    [cr_block_instru2_max, n_block_instru2_max] =...
                max(cr_block_instru2);
            [cr2_block_instru_max, n2_block_instru2_max] =...
	        max(cr_block_instru2_max);
	    p_block_instru_max =...
	        p_block_instru2(n_block_instru2_max(n2_block_instru2_max));

	    %Ajout aux statistiques, delta temporel (échantillon et 
	    %secondes), corrélation.

	    %Calcul du range
	    n_max_corr_start = 1 - p_block_instru_max;
	    n_max_corr_end   = n_max_corr_start + n_block - 1;
	    rg_max_corr = max(1,n_max_corr_start):min(n_instru2,n_max_corr_end);


	    %Calcul du coefficient.
	    coefficient = cv_block_instru2(n2_block_instru2_max, :)...
	    		./(nm_block.* nm_instru2_mp(n2_block_instru2_max,:));
            if(length(rg_block) ==length(rg_max_corr ))
		%Ajout du bloc au flux.
		au_vox(rg_block,:) =    au_full(rg_block,:)...
				      - coefficient...
				     .* au_instru2(rg_max_corr,:);
            end
    end
end
%Écriture du fichier.
audiowrite([tx_dst,tx_song,tx_vox,tx_wav],au_vox,f_samplerate,'BitsPerSample',32)
